...
export default function ReconcilePage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-[60vh]"><div className="text-gray-400 dark:text-zinc-500">Chargement...</div></div>;
  }

  if (!monthData) {
    return <div className="flex items-center justify-center h-[60vh]"><div className="text-gray-400 dark:text-zinc-500">Mois introuvable</div></div>;
  }
  ...
  return (
    <div className="max-w-[900px] mx-auto px-4 py-5 sm:p-6 lg:p-8">
      {/* Header */}
      <div className="mb-6">
        <button onClick={() => router.push(`/month/${monthId}`)} className="text-sm text-gray-500 dark:text-zinc-500 hover:text-gray-700 dark:hover:text-zinc-300 mb-2 block">
          &larr; Retour au mois
        </button>
        ...
        {[
          ...
          <div
            ...
            className={`flex-1 text-center text-sm font-medium py-2 rounded-lg border transition-colors ${
              step === n
                ? "bg-gray-100 dark:bg-zinc-800 text-gray-900 dark:text-zinc-200 border-gray-300 dark:border-zinc-700"
                : step > n
                ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20"
                : "bg-gray-50 dark:bg-zinc-900 text-gray-400 dark:text-zinc-600 border-gray-200 dark:border-zinc-800"
            }`}
          >
            <span className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-xs mr-1.5 ${
              step > n ? "bg-emerald-500 text-white dark:text-emerald-950" : step === n ? "bg-gray-900 dark:bg-zinc-200 text-white dark:text-zinc-900" : "bg-gray-200 dark:bg-zinc-800 text-gray-500 dark:text-zinc-500"
            }`}>
              {step > n ? "\u2713" : n}
            </span>
            {label}
      ...
      {step === 2 && result && monthData && (
        <>
          ...
          <div className="border border-gray-200 dark:border-zinc-800 rounded-xl mb-5 overflow-hidden">
            ...
            <div className="border-t border-gray-200 dark:border-zinc-800 px-4 py-4">
              <div className="grid grid-cols-3 gap-4 mb-3">
                <div>
                  <div className="text-xs text-gray-500 dark:text-zinc-500 mb-1">Budget prévu</div>
                  <div className="text-lg font-bold">{formatEur(result.everydayLifeComparison.budgeted)}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-zinc-500 mb-1">Dépenses relevé</div>
                  <div className="text-lg font-bold text-red-600 dark:text-red-400">{formatEur(result.everydayLifeComparison.actual)}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-zinc-500 mb-1">Écart</div>
                  <div className={`text-lg font-bold ${result.everydayLifeComparison.gap > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}>
          ...
          {/* Validate bar */}
          <div className="sticky bottom-0 bg-white/95 dark:bg-zinc-950 border-t border-gray-200 dark:border-zinc-800 py-4 mt-6 flex items-center justify-between">
            <span className="text-sm text-gray-500 dark:text-zinc-400">
              <strong className="text-gray-900 dark:text-zinc-200">{progress.done}</strong> / {progress.total} traitées
            </span>
            <button
              onClick={handleSave}
              ...
              className={`text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors ${
                allDone && !saving
                  ? "bg-emerald-500 text-white dark:text-emerald-950 hover:bg-emerald-400"
    ...
    >
      {uploading ? (
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-gray-500 dark:text-zinc-400">Analyse en cours...</p>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-2">
          <span className="text-3xl opacity-50">📄</span>
          <p className="text-sm text-gray-500 dark:text-zinc-400">
            <span className="text-blue-500 dark:text-blue-400 font-medium">Cliquez</span> ou glissez votre relevé bancaire CSV
          </p>
          <p className="text-xs text-gray-400 dark:text-zinc-600">Format : Date ; Libellé ; Montant ; Catégorie (séparateur « ; », date ISO ou JJ/MM/AAAA)</p>
        </div>
      )}
    </div>
  );
...
function StatCard({ value, label, color }: { value: number; label: string; color: string }) {
  ...
  return (
    <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-lg p-3 text-center">
      <div className={`text-xl font-bold ${colorMap[color]}`}>{value}</div>
      <div className="text-xs text-gray-500 dark:text-zinc-500 mt-0.5">{label}</div>
    </div>
  );
}
...
function CollapsibleSection({ title, count, dotColor, defaultOpen, children }: {
  ...
  return (
    <div className="border border-gray-200 dark:border-zinc-800 rounded-xl mb-5 overflow-hidden">
      <div
        className="px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-900"
        onClick={() => setOpen(!open)}
      >
        <h2 className="text-sm font-semibold flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${dotColor}`} />
          {title}
          <span className="bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-zinc-400 text-xs px-2 py-0.5 rounded-full">{count}</span>
        </h2>
        <span className={`text-gray-400 dark:text-zinc-600 text-xs transition-transform ${open ? "rotate-180" : ""}`}>▼</span>
      </div>
      {open && <div className="border-t border-gray-200 dark:border-zinc-800 divide-y divide-gray-100 dark:divide-zinc-800/50">{children}</div>}
    </div>
  );
}
  ...
  return (
    <div className="flex items-center px-4 py-2.5 gap-3">
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-gray-400 dark:text-zinc-600">{match.bankLine.date}</div>
      </div>
      ...
      <span className="text-xs bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded whitespace-nowrap">
    ...
    return (
      <div className="flex items-center px-4 py-2.5 gap-3">
        ...
        {confirmed ? (
          ...
          <span className="text-xs bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-zinc-500 px-2 py-0.5 rounded whitespace-nowrap">
            ✕ Pas le bon
          </span>
        )}
      </div>
  ...
  return (
    <div>
      <div className="flex items-center px-4 py-2.5 gap-3">
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium truncate">{match.bankLine.label}</div>
          <div className="text-xs text-gray-400 dark:text-zinc-600">{match.bankLine.date} &middot; montant correspond à &quot;{match.matchedExpenseLabel}&quot;</div>
        </div>
        <div className="text-sm font-semibold text-red-600 dark:text-red-400 tabular-nums whitespace-nowrap">
          {formatEur(match.bankLine.amount)}
        </div>
        <span className="text-xs bg-yellow-50 dark:bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 px-2 py-0.5 rounded whitespace-nowrap">
      ...
      <div className="px-4 pb-3 pl-8 flex items-center gap-3 flex-wrap">
        ...
        <div className="flex gap-1.5 ml-auto">
          <button
            ...
            className="text-xs px-2.5 py-1 rounded border transition-colors bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 border-gray-300 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800"
          >
            ✓ Confirmer
          </button>
          <button
            ...
            className="text-xs px-2.5 py-1 rounded border bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 border-gray-300 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800"
          >
            ✕ Pas le bon
          </button>
        </div>
...
function UnmatchedItem({ match, decision, onDecision, expenses }: {
  ...
  return (
    <div>
      <div className={`flex items-center px-4 py-2.5 gap-3 ${isIgnored || isPunctual ? "opacity-50" : ""} ${isLinked ? "border-l-2 border-emerald-500 dark:border-emerald-400" : ""}`}>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium truncate">{match.bankLine.label}</div>
          <div className="text-xs text-gray-400 dark:text-zinc-600">{match.bankLine.date} &middot; {match.bankLine.category}</div>
        </div>
        <div className="text-sm font-semibold text-red-600 dark:text-red-400 tabular-nums whitespace-nowrap">
          {formatEur(match.bankLine.amount)}
        </div>
        ...
        {isPunctual && (
          <span className="text-xs bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded whitespace-nowrap">
            ● Ponctuel
          </span>
        )}
        <div className="flex gap-1 flex-shrink-0">
          <button
            ...
            className={`text-xs px-2 py-1 rounded border transition-colors ${
              isLinked ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-300 dark:border-emerald-500/30" : "bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 border-gray-300 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800"
            }`}
          >
            🔗 Lier
          </button>
          <button
            ...
            className={`text-xs px-2 py-1 rounded border transition-colors ${
              isIgnored ? "bg-gray-200 dark:bg-zinc-800 text-gray-500 dark:text-zinc-500 border-gray-300 dark:border-zinc-600" : "bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 border-gray-300 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800"
            }`}
          >
            Ignorer
          </button>
          <button
            ...
            className={`text-xs px-2 py-1 rounded border transition-colors ${
              isPunctual ? "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-300 dark:border-blue-500/30" : "bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 border-gray-300 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800"
            }`}
          >
            Ponctuel
          </button>
          <button
            ...
            className={`text-xs px-2 py-1 rounded border transition-colors ${
              isRecurring ? "bg-yellow-50 dark:bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 border-yellow-300 dark:border-yellow-500/30" : "bg-gray-50 dark:bg-zinc-900 text-gray-500 dark:text-zinc-400 border-gray-300 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800"
            }`}
          >
            ★ Récurrent
          </button>
      ...
      {/* Link search panel */}
      {linkOpen && !isLinked && (
        <div ref={linkPanelRef} className="px-4 pb-3 pl-8 bg-gray-50/50 dark:bg-zinc-900/50">
          <div className="text-xs text-gray-400 dark:text-zinc-600 mb-1.5">Rechercher une dépense existante :</div>
          <SearchSelect
            items={expenses}
            filterFn={(e, q) => {
            ...
            renderItem={(exp, highlighted) => (
              <div className={`flex items-center justify-between px-3 py-2 ${highlighted ? "bg-gray-100 dark:bg-zinc-800" : "hover:bg-gray-100 dark:hover:bg-zinc-800"}`}>
                <div>
                  <div className="text-xs font-medium">{exp.label}</div>
                  <div className="text-xs text-gray-400 dark:text-zinc-600">{exp.category || "Sans catégorie"} &middot; {exp.type}</div>
                </div>
                <div className="text-xs font-semibold text-red-600 dark:text-red-400">{formatEur(-exp.amount)}</div>
              </div>
            )}
            onSelect={handleLink}
            keyFn={(e) => e.id}
      ...
      {isRecurring && formConfirmed && (
        <div className="flex items-center px-4 py-2 pl-8 gap-2">
          <span className="text-xs px-2 py-0.5 rounded bg-yellow-50 dark:bg-yellow-500/10 text-yellow-600 dark:text-yellow-400">
            ★ Récurrent
          </span>
          <span className="text-xs text-gray-700 dark:text-zinc-300">{newLabel}</span>
          <span className="text-xs text-gray-500 dark:text-zinc-500">{formatEur(newAmount)}</span>
          {newCategory && <span className="text-xs text-gray-400 dark:text-zinc-600">{newCategory}</span>}
          <button
            onClick={() => setFormConfirmed(false)}
            className="text-xs text-gray-400 dark:text-zinc-600 hover:text-gray-600 dark:hover:text-zinc-400 ml-auto"
          >
            Modifier
          </button>
        </div>
      ...
      {showForm && (
        <div className="px-4 pb-3 pl-8 bg-gray-50/50 dark:bg-zinc-900/50">
          <div className="flex gap-2 items-center flex-wrap">
            <input
              ...
              className="flex-1 min-w-[150px] bg-white dark:bg-zinc-950 border border-gray-300 dark:border-zinc-700 rounded-md px-2.5 py-1.5 text-sm text-gray-900 dark:text-zinc-200 focus:outline-none focus:border-gray-500 dark:focus:border-zinc-500"
              placeholder="Label"
              autoFocus
            />
            <input
              ...
              className="w-20 bg-white dark:bg-zinc-950 border border-gray-300 dark:border-zinc-700 rounded-md px-2.5 py-1.5 text-sm text-gray-900 dark:text-zinc-200 focus:outline-none focus:border-gray-500 dark:focus:border-zinc-500"
              step="0.01"
            />
            <input
              ref={categoryRef}
              ...
              className="w-28 bg-white dark:bg-zinc-950 border border-gray-300 dark:border-zinc-700 rounded-md px-2.5 py-1.5 text-sm text-gray-900 dark:text-zinc-200 focus:outline-none focus:border-gray-500 dark:focus:border-zinc-500"
...
function ReconciliationBilan({ result, decisions, monthData, onGoToMonth }: {
  ...
  const scoreColor = pourcentageErreur <= 5 ? "text-emerald-600 dark:text-emerald-400" : pourcentageErreur <= 15 ? "text-yellow-600 dark:text-yellow-400" : "text-red-600 dark:text-red-400";
  ...
  return (
    <div className="space-y-5">
      {/* Score principal */}
      <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-6 text-center">
        <div className="text-xs text-gray-500 dark:text-zinc-500 uppercase tracking-wider mb-2">Précision des prévisions</div>
        ...
        <div className="mt-4 h-2 bg-gray-200 dark:bg-zinc-800 rounded-full overflow-hidden max-w-xs mx-auto">
          <div
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4">
          <div className="text-xs text-gray-500 dark:text-zinc-500 mb-1">Dépenses prévues</div>
          <div className="text-xl font-bold">{formatEur(totalPrevu)}</div>
        </div>
        <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4">
          <div className="text-xs text-gray-500 dark:text-zinc-500 mb-1">Dépenses réelles</div>
          <div className="text-xl font-bold text-red-600 dark:text-red-400">{formatEur(totalReel)}</div>
        </div>
        <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4">
          <div className="text-xs text-gray-500 dark:text-zinc-500 mb-1">Écart</div>
          <div className={`text-xl font-bold ${ecart > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}>
            {ecart > 0 ? "+" : ""}{formatEur(ecart)}
          </div>
        </div>
        <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4">
          <div className="text-xs text-gray-500 dark:text-zinc-500 mb-1">Transactions non prévues</div>
          <div className={`text-xl font-bold ${transactionsNonPrevues > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}>
            {transactionsNonPrevues}
          </div>
          <div className="text-xs text-gray-400 dark:text-zinc-600 mt-0.5">{transactionsMatchees} / {totalTransactions} matchées ({tauxCouverture.toFixed(0)}%)</div>
        </div>
      </div>
      ...
      {result.everydayLifeComparison.gap !== 0 && (
        <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4">
          <div className="text-xs text-gray-500 dark:text-zinc-500 mb-2">Every day life</div>
          <div className="flex items-baseline gap-3">
            <span className="text-sm text-gray-500 dark:text-zinc-400">Budget {formatEur(result.everydayLifeComparison.budgeted)}</span>
            <span className="text-gray-300 dark:text-zinc-600">→</span>
            <span className="text-sm">Réel {formatEur(result.everydayLifeComparison.actual)}</span>
            <span className={`text-sm font-semibold ${result.everydayLifeComparison.gap > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}>
              ({result.everydayLifeComparison.gap > 0 ? "+" : ""}{formatEur(result.everydayLifeComparison.gap)})
            </span>
          </div>
        </div>
      ...
      {/* Actions effectuées */}
      <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4">
        <div className="text-xs text-gray-500 dark:text-zinc-500 uppercase tracking-wider mb-3">Actions effectuées</div>
        <div className="space-y-1.5 text-sm">
          {recurring.length > 0 && (
            <div className="text-yellow-600 dark:text-yellow-400">★ {recurring.length} nouvelle(s) récurrente(s) ajoutée(s) pour le mois prochain</div>
          )}
          {punctual.length > 0 && (
            <div className="text-blue-600 dark:text-blue-400">● {punctual.length} dépense(s) ponctuelle(s) enregistrée(s)</div>
          )}
          {linked.length > 0 && (
            <div className="text-emerald-600 dark:text-emerald-400">🔗 {linked.length} dépense(s) liée(s) manuellement</div>
          )}
          {ignored.length > 0 && (
            <div className="text-gray-500 dark:text-zinc-500">● {ignored.length} ligne(s) ignorée(s)</div>
          )}
          {bankLabelsLearned > 0 && (
            <div className="text-emerald-600 dark:text-emerald-400">✓ {bankLabelsLearned} libellé(s) bancaire(s) appris pour les prochains mois</div>
          )}
        </div>
      </div>
      ...
      <button
        onClick={onGoToMonth}
        className="w-full bg-emerald-500 text-white dark:text-emerald-950 font-semibold py-2.5 rounded-lg hover:bg-emerald-400 transition-colors"
      >
        Retour au mois →
      </button>
    </div>
